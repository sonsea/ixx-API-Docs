...
- 字段说明
  - id 唯一编号
  - name 合约名(EHTUSD,EOSUSD),永续合约接口里传的currency参数和symbol参数都改成这个合约名name做为参数
  - symbol 交易对名字 例:UNIT_ETH
  - product UNIT
  - currency 币种(ETH,EOS)
  - amount_scale 数量精度
  - price_scale 价格精度
...
- 返回值
  - code(200表示正常读取data内容，非200则表示失败读取message失败信息)
  - data 详见 /unit/account/balance/list
  - message
- 字段说明
- 限定访问间隔时间
	-	1000毫秒
...
- 字段说明
    ...
    - name 合约名(EHTUSD,EOSUSD)
    - side 1买 2卖
    - type 1限价 2市价 3限价止损 4市价止损 5限价止盈 6限价止盈
    - price 成交价
    - amount 成交量
    - amount_total 委托数量
    - amount_last 成交量
